...
select ns.nspname as SchemaName,
       ...
       array_remove(array_agg(a.attname order by x.ordinality), null) as ColumnNames
from pg_class t
...
order by ns.nspname, t.relname, i.relname;";
        ...
        var columns = (await db.QueryAsync<ColumnRow>(columnsSql)).ToArray();
        var primaryKeys = (await db.QueryAsync<PrimaryKeyRow>(primaryKeysSql)).ToArray();
        var foreignKeys = (await db.QueryAsync<ForeignKeyRow>(foreignKeysSql)).ToArray();
        var indexes = (await db.QueryAsync<IndexRow>(indexesSql)).ToArray();
        ...
        var indexesByTable = indexes
            ...
            .ToDictionary(
                x => x.Key,
                x => (IReadOnlyCollection<IndexSnapshotNode>)x.Select(index => new IndexSnapshotNode
                {
                    Name = index.IndexName,
                    IsUnique = index.IsUnique,
                    ColumnNames = (index.ColumnNames ?? Array.Empty<string>()).Where(name => !string.IsNullOrWhiteSpace(name)).ToArray(),
                }).ToArray());
...
order by table_schema, table_name, ordinal_position;";
    ...
    private sealed record IndexRow(string SchemaName, string TableName, string IndexName, bool IsUnique, string[]? ColumnNames);
}
